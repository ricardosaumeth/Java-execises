
public class Main {
    public static void main(String[] args) {

//        int[] result = new SumOfTwo().twoSum(new int[]{2,7,11,15}, 9);
//        boolean result = new IsPalindromeReverseTheString().isPalindromeReverseTheString("121"); // -121
//        int result = new RomanToInteger().romanToInt("XL");
//          String result = new LongestCommonPrefix().longestCommonPrefix(new String[]{"flower","flow","flight"});
//
        //boolean result = new ValidParentheses().isValid("(]");
       // int result = new RemoveDuplicates().removeDuplicates(new int[]{ 1, 2, 2, 3, 3, 4, 4, 4, 5, 5, 6 } );
        int result = new FindIndexFirstOccurrence().firstOccurrence("leetcode", "leeto");
        System.out.println(result);
    }
}